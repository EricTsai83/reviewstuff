...
`ensureSupportedFakeSelection`（hardcoded fake triple），並把 `UnsupportedReviewSelectionError` 從
`config/config-service.ts` 搬到 engines（它是 engine capability error，不是 config parsing error）；
