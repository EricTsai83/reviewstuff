...
- extend `src/platform/command-runner.ts` with its canonical implementation and `layer`
...
- 完成 004 定義的 `CommandRunner` canonical implementation：timeout、combined output
  limit、stdout/stderr、exit code、cancellation cleanup
- `GitService` implementation 只透過 `CommandRunner` 執行 git，不得直接使用
...
1. 在 `platform/command-runner.ts` 使用 `@effect/platform/Command.start` 實作
   canonical `CommandRunner` layer；並行消耗
...
2. 在 `git/git-service.ts` 定義不暴露 `CommandRunner` 或 platform types 的
   `GitService` contract，並在同一 canonical module 建立依賴 `CommandRunner` 的
   implementation 與 `layer`；`runReview` 只依賴 `GitService`。
