...
```bash
reviewstuff review --staged --engine openai --model <model-id> --json
reviewstuff review --staged --engine anthropic --model <model-id> --json
reviewstuff review --staged --engine codex-cli --model <model-id> --json
```
...
- Anthropic cloud API: 透過 API key 呼叫指定 model，回傳 structured JSON findings。
- Codex CLI/local provider: 透過本機 provider CLI 呼叫指定 model，回傳 structured JSON findings。
...
3. 實作 OpenAI adapter。
4. 實作 Anthropic adapter。
5. 實作 Codex CLI/local adapter。
6. 將 `review --engine <id> --model <model-id>` 接到 real provider。
7. provider error 轉成清楚的 CLI error 與 doctor diagnostics。

## Verification

```bash
bun run test
AI_REVIEW_ENGINE=openai AI_REVIEW_MODEL=<model-id> ./dist/reviewstuff review --staged --json
AI_REVIEW_ENGINE=anthropic AI_REVIEW_MODEL=<model-id> ./dist/reviewstuff review --staged --json
AI_REVIEW_ENGINE=codex-cli AI_REVIEW_MODEL=<model-id> ./dist/reviewstuff review --staged --json
...
- OpenAI、Anthropic、local CLI provider 都走同一個 `ReviewEngine` contract。
