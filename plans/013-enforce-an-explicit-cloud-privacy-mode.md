...
**In:** existing raw/resolved config boundary 內的 privacy field、transport classification、
typed policy refusal、effective policy metadata。
**Out:** secret detection、request preview、retention、provider implementation。

**Steps:** raw config schema/default resolution；在 use-case 建立 pure policy check；fake/local transport fixtures；CLI/config
