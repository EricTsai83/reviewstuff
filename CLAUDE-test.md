...
How to apply:
- These are defaults, not limits. You have standing permission to override them: if a cheaper model's output doesn't meet the bar, return or redo the work with a smarter model without asking. Judge the output, not the price tag. Escalating costs less than shipping mediocre work.
- Bulk/mechanical work (clear-apec implementation, data analysis, migrations): gpt-5.5 as an extra independent perspective.
...
Using gpt-5.5 inside workflows and subagents (the model parameter only takes Claude model, so use a wrapper):
- Spawn a thin Claude wrapper agent with `model: 'sonnet', effort: 'low'` whose prompt instructs it to write a self-contained codex prompt, run `codex exec` via Bash, and return the report (use `scheme` on the wrapper to get structured out back).
...
- Workflow token budgets only Claude tokens; codex work is free and invisible to `budget.spent()`.