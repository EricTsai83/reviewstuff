...
用 `reviewstuff` CLI 對目前的變更做跨模型審查（審查模型 ≠ 撰寫模型）。

1. 跑 `reviewstuff --staged --json`。在 Claude Code 裡執行時，確保審查方不是 Claude：
...
常見解法：`reviewstuff doctor`、`reviewstuff login openai-codex`）· `3` 執行期失敗（引擎全掛）。
...
- 想自動修而非手動修：`reviewstuff fix --staged --apply`（會在暫存 worktree 跑
  設定檔的 gates 驗證，全綠才寫回；沒設 gates 就直接套用，風險自負）。
